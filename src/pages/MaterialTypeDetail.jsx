...
export default function MaterialTypeDetail() {
  ...
  if (!materialType && !family) return <Navigate to={`/products/${catSeoSlug(category.slug)}`} replace />;

  const active = family || materialType;
  const related = productCategories.filter((c) => c.slug !== slug).slice(0, 3);
  const combinedGrades = active.grades?.join(", ");
  const activeSizes = active.sizes || category.sizes;
  const activeForm = active.form || category.form;
  const activeStandards = active.standards || category.standards;
  const pageDescription = `${active.desc || category.short} Available grades: ${combinedGrades}. Size range ${activeSizes}, meeting standards ${(activeStandards || []).join(", ")}.`;

  return (
    <>
      <SEO
        title={`${active.name} | ${category.name}`}
        description={pageDescription}
        keywords={`${active.name.toLowerCase()}, ${combinedGrades?.toLowerCase()}, ${category.name.toLowerCase()}`}
        type="product"
      />
      <OrganizationLd />
      <WebPageLd
        url={absoluteUrl(`/products/${catSeoSlug(category.slug)}/${seoSlug(active.name)}`)}
        title={`${active.name} | ${category.name}`}
        description={pageDescription}
      />
      <BreadcrumbLd
        items={[
          ...
          { name: active.name, url: absoluteUrl(`/products/${catSeoSlug(category.slug)}/${seoSlug(active.name)}`) },
        ]}
      />
      ...
      <section className="bg-paper border-b border-teal-pale/50">
        <div className="max-w-7xl px-6 py-3 text-xs">
          ...
          <span className="text-steel-dark mx-2">/</span>
          <span className="text-deep font-semibold">{active.name}</span>
        </div>
      </section>
      ...
      <section className="bg-paper py-10">
        <div className="max-w-7xl px-6 grid lg:grid-cols-[260px_1fr] gap-10">
          {/* =============== LEFT SIDEBAR — 2 widgets like ProductDetail ============ */}
          <aside className="space-y-6">
            <CategorySidebar category={category} activeTypeSlug={typeSlug} />
          </aside>
          ...
          <div>
                ...
                <div className="flex flex-col sm:flex-row sm:flex-wrap gap-x-10 gap-y-4 font-mono text-[12.5px] tracking-[0.06em]">
                  {activeSizes && (
                    <div className="flex items-baseline gap-3">
                      ...
                      <span className="text-paper font-bold leading-relaxed">{activeSizes}</span>
                    </div>
                  )}
                  {activeForm && (
                    <div className="flex items-baseline gap-3">
                      ...
                      <span className="text-paper font-bold leading-relaxed">{activeForm}</span>
                    </div>
                  )}
                  {combinedGrades && (
                    <div className="flex items-baseline gap-3 sm:col-span-2 w-full">
                      ...
                      <span className="text-paper font-bold leading-relaxed">{combinedGrades}</span>
                    </div>
                  )}
                  {activeStandards?.length && (
                    <div className="flex items-baseline gap-3 sm:col-span-2 w-full">
                      ...
                      <span className="text-paper font-bold leading-relaxed">{activeStandards.join(", ")}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            {/* ---- 2b. Image + desc row (if family has real photo) -- */}
            {active.image && (
              <Reveal className="mt-12">
                <div className="grid md:grid-cols-[1.05fr_1fr] gap-10 items-start">
                  ...
                  >
                    <ProductImage
                      category={{ image: active.image, name: active.name }}
                  ...
                  {family.variants.map((v, i) => (
                    <Reveal key={v.name} delay={i * 0.05}>
                      <Link
                        ...
                        className="group block bg-white border border-teal-pale rounded-lg p-7 sm:p-8 hover:shadow-lg hover:shadow-teal/10 hover:border-teal transition-all duration-300"
                      >
                        <div className="flex items-start gap-3">
                          <ChevronRight size={20} className="text-teal shrink-0 mt-1 group-hover:translate-x-1 transition-transform" />
                          <div className="flex-1 min-w-0">
                            <h3 className="font-display uppercase text-lg sm:text-xl text-deep group-hover:text-teal transition-colors">{v.name}</h3>
                            <div className="mt-1 h-px w-16 bg-teal" />
                            <p className="mt-4 text-sm sm:text-[15px] text-[#333] leading-relaxed">{v.desc}</p>
                            ...
                            <span className="mt-5 inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-teal">
            ...
            {([...(active.specTables || []), ...(category.specTables || [])].length > 0) && (
              <div className="mt-16 space-y-10">
                ...
                {[...(active.specTables || []), ...(category.specTables || [])].map((tbl, i) => (
            ...
            {/* ---- 4b. Sibling types (other materials in same category) ------------- */}
            {materialTypes.length > 1 && (
              <div className="mt-20">
                <Reveal className="mb-10">
                  <span className="text-xs uppercase tracking-[0.25em] text-teal font-semibold">More In This Line</span>
                  <h2 className="font-display uppercase text-2xl sm:text-3xl mt-3 text-deep leading-tight">
                    Other {category.name} Types
                  </h2>
                  <div className="mt-2 h-[3px] w-20 bg-teal" />
                </Reveal>

                <div className="grid sm:grid-cols-2 gap-5">
                  {materialTypes.filter((mt) => mt.name !== active.name).map((mt, i) => (
                    <Reveal key={mt.name} delay={i * 0.05}>
                      <Link
                        to={`/products/${catSeoSlug(category.slug)}/${seoSlug(mt.name)}`}
                        className="group flex items-center justify-between gap-3 bg-white border border-teal-pale rounded-lg px-6 py-5 hover:border-teal hover:shadow-lg hover:shadow-teal/10 transition-all duration-300"
                      >
                        <div className="flex-1 min-w-0">
                          <span className="font-display uppercase text-sm text-deep group-hover:text-teal transition-colors">
                            {mt.name}
                          </span>
                          {mt.variants?.length ? (
                            <span className="ml-2 text-[11px] font-semibold uppercase tracking-wide text-steel-dark">
                              {mt.variants.length} grades
                            </span>
                          ) : null}
                        </div>
                        <ArrowRight size={16} className="shrink-0 text-teal opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all" />
                      </Link>
                    </Reveal>
                  ))}
                </div>
            ...
            <Reveal className="mt-14">
              <div className="border-t border-gray-200 pt-10">
                <h3 className="font-display uppercase text-[13.5px] font-bold text-[#111] tracking-[0.05em] leading-snug">
                  EXPORT DESTINATIONS FOR {active.name.toUpperCase()}, {category.grades?.slice(0,3).join(", ").toUpperCase()} {category.name.toUpperCase()}
                </h3>
